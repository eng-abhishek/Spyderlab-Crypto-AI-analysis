...
<div class="d-flex align-items-center mb-3">
	<div>
		<ul class="breadcrumb">
			...
			<li class="breadcrumb-item active">{{strtoupper('NewsLetter Emails')}}</li>
		</ul>                    
	</div>
	<div class="ms-auto">
...
<script type="text/javascript">
	$(document).ready(function(){
		...
		var table = $('#m_table_1').DataTable({
			...
			responsive: true,
			ajax:{
				url:"{{ route('backend.posts.newsletter-email.index') }}",
				data: function (d) {
			...
			if(id.length > 0)
			{
				...
				Swal.fire({
					...
					if (result.value){
						...
						$.ajax({
							url:"{{ route('backend.posts.removeall-newsletter-email')}}",
