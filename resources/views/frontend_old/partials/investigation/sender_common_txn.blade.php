<div class="table-responsive">
	<table class="table align-middle txn-table">
		...
		<tbody>
			@foreach($blockcypher_txn_sender as $txn)
			<tr>
				<td>
					<p class="mb-0 text-truncate txn-address or_txn_address{{$txn->id}}">{{$txn->from}}</p>
					<p>
						<a href="javascript:void(0)"><i class="fa-light fa-copy me-1" onclick="copyToClipboard('.or_txn_address','{{$txn->id}}')"></i></a>
						<a href="{{route('blockchain-search').'?keyword='.$txn->from}}" target="_blank"><i class="fa-light fa-external-link"></i></a>
					</p>
				</td>
				<td>
